...
export function filterPlaces(places, filters, userPosition) {
  const search = String(filters.search || '').trim().toLowerCase();
  const zone = filters.zone === 'all' ? '' : String(filters.zone || '').trim().toLowerCase();
...
export function usePlaceFilters(places, filters, userPosition) {
  return useMemo(() => filterPlaces(places, filters, userPosition), [filters, places, userPosition]);
}
